...
def _load_config():
    ...
    config = {
        "total_accounts": 3,
        "mail_provider": "tempmail",          # duckmail | tempmail
...
MAIL_PROVIDER = _CONFIG.get("mail_provider", "tempmail").lower().strip()
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='ChatGPT 批量注册工具')
    parser.add_argument('--batch', action='store_true', help='非交互批量模式')
    parser.add_argument('--count', type=int, default=None, help='注册数量')
    parser.add_argument('--workers', type=int, default=3, help='并发数')
    args = parser.parse_args()

    if args.batch:
        count = args.count or int(os.environ.get('TOTAL_ACCOUNTS', DEFAULT_TOTAL_ACCOUNTS))
        proxy = os.environ.get('PROXY', DEFAULT_PROXY) or None
        output = os.environ.get('OUTPUT_FILE', DEFAULT_OUTPUT_FILE)
        run_batch(total_accounts=count, output_file=output, max_workers=args.workers, proxy=proxy)
    else:
        main()
