...
/**
 * 批量删除指定的提供商配置（按 UUID 列表）
 */
export async function handleBatchDeleteProviders(req, res, currentConfig, providerPoolManager, providerType) {
    try {
        const body = await getRequestBody(req);
        const { uuids } = body;
        ...
        const remainingProviders = providers.filter(p => !uuidSet.has(p.uuid));
        
        if (deletedProviders.length === 0) {
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({
                success: true,
                message: 'No matching providers to delete',
                deletedCount: 0,
                remainingCount: providers.length
            }));
            return true;
        ...
        // Save to file
        writeFileSync(filePath, JSON.stringify(providerPools, null, 2), 'utf-8');
        logger.info(`[UI API] Batch deleted ${deletedProviders.length} providers from ${providerType}`);
        ...
        res.end(JSON.stringify({
            success: true,
            message: `Successfully deleted ${deletedProviders.length} providers`,
            deletedCount: deletedProviders.length,
